...
def create_comment(task_id: int, comment: CommentCreate, service: CommentService = Depends(get_comment_service)):
    return service.create_comment(task_id, comment)


@router.get("/{task_id}/comments", response_model=list[CommentResponse], dependencies=[Depends(get_current_user)])
def get_comments(task_id: int, service: CommentService = Depends(get_comment_service)):
