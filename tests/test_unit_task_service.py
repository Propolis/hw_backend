...
def test_create_task():
    ...
    result = asyncio.run(service.create_task(TaskCreate(title="Test task")))
...
def test_create_task_with_description():
    ...
    service = TaskService(mock_repo)
    result = asyncio.run(service.create_task(TaskCreate(title="With desc", description="desc")))
...
def test_get_task_not_found():
    ...
    with pytest.raises(TaskNotFound) as exc_info:
        asyncio.run(service.get_task(42))
...
def test_delete_task_not_found():
    ...
    with pytest.raises(TaskNotFound):
        asyncio.run(service.delete_task(99))
